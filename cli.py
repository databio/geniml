...
def main():
    ...
    if not args.embed_file:
        ...
        embeddings_dictfile = os.path.join(args.output, args.title + "_embeddings.pkl")
        save_dict(embeddings, embeddings_dictfile)
        embeddings_csvfile = os.path.join(args.output, args.title + "_embeddings.csv")
        (
            pd.DataFrame.from_dict(data=embeddings, orient="index").to_csv(
                embeddings_csvfile, header=False
            )
        )
    ...
    coordinates_csvfile = os.path.join(args.output, args.title + "_xy_coords.csv")
    plot_name = (
        f"_nocells{args.nocells}_noreads{args.noreads}_"
        f"dim{args.dimension}_win{args.window_size}_"
        f"mincount{args.min_count}_"
