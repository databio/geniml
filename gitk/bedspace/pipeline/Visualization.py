...
def retrieve_meta_test():
    meta_test = ((pd.read_csv(path_meta+'tests/test_file_meta.csv'))[['file_name', 'cell_type', 'target']])
...
def Scenario1(path_simfile):
    ...
    for searchterm in training_labels:
        ...
        df=search_table[['filename', 'file_label', 'original_label', searchterm]].sort_values(by=[searchterm])[0:10]
        ...
        plt.figure.savefig('../outputs/bedembed_output/figures/S1/{}_nof{}.svg'.format(searchterm, nof), format = 'svg', bbox_inches='tight')
...
meta_train = ((pd.read_csv(path_meta+'tests/test_file_meta.csv'))[['file_name', 'cell_type', 'target']])
...
sim[['train_name', 'train_label_cell', 'train_label_target']]=sim.db_file.str.split(',', expand=True)[[0,1, 2]]
sim[['test_name', 'test_label_cell', 'test_label_target']]=sim.test_file.str.split(',', expand=True)[[0,1, 2]]

sim = sim.merge(meta_test, left_on = 'test_name', right_on = 'file_name').merge(meta_train, left_on = 'train_name', right_on = 'file_name', suffixes=('_test', '_train'))#

sim.score = 1 - sim.score

for test in list(set(sim.test_name)):
    
    df = sim[sim.test_name==test].sort_values(by ='score', ascending = False)[['test_name', 'train_name', 'original_label_test', 'original_label_train', 'score']]
    ...
    plt= df.plot.barh(x='original_label_train', y='score', figsize=(10,7), fontsize=16, color=list(df['color']))
    plt.axis(xmin=0.7,xmax=1.01)
    
    plt.figure.savefig('../outputs/bedembed_output/figures/S3/{}_nof{}.svg'.format(test.split('/')[-1], nof), format = 'svg', bbox_inches='tight')
