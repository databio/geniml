...
def run_scenario1(
    ...
    df['color']='green'
    plt= df.plot.barh(x='filename', y=searchterm, figsize=(6,4), fontsize=10, color=list(df['color']))
    plt.set_xlabel('Similarity', fontsize=10)
    plt.set_ylabel('File_name', fontsize=10)
    plt.axis(xmin=0.0, xmax=1.01)

    plt.figure.savefig('{}/{}_nof{}.svg'.format(output, query, num_results), format = 'svg', bbox_inches='tight')
...
def run_scenario2(
    ...
    df['color']='green'
    plt= df.plot.barh(x='search_term', y='score', figsize=(6,4), fontsize=10, color=list(df['color']))
    plt.set_xlabel('Similarity', fontsize=10)
    plt.set_ylabel('Ranked labels', fontsize=10)
    plt.axis(xmin=0.0, xmax=1.01)

    plt.figure.savefig('{}/{}_nof{}.svg'.format(output, query.split('/')[-1], num_results), format = 'svg', bbox_inches='tight')
...
def run_scenario3(
    ...
    distance = distance.drop_duplicates()
    df = distance[distance.test_file.str.contains(file)].sort_values(by = ['score'], ascending = False)[0:num_results]
    df = df.sort_values(by=['score'], ascending=True)

    df['color']='green'
    plt= df.plot.barh(x='db_file', y='score', figsize=(6,4), fontsize=10, color=list(df['color']))
    plt.set_xlabel('Similarity', fontsize=10)
