...
def bedtool_tokenization(f, bedtool_path, data_folder, target_folder, universe, fraction):
        ...
        with open(target, 'w') as f_target:
            subprocess.run(shlex.split("{} intersect -a {} -b {} -u -f {} ".format(bedtool_path,universe,temp, fraction)),stdout=f_target)
        os.remove(temp)
...
def generate_tokens(raw_data_folder, token_folder, universe, file_list, bedtool, fraction):
    ...
    if os.path.exists(token_folder):
        ...
        if number == 0:
            ...
            print('Folder {} exists with {} files not processed. Continue...'.format(token_folder,number),flush=True)
    else:
...
def main(args):
    ...
    status = generate_tokens(args.data_folder, args.token_folder, args.universe, args.file_list, args.bedtools_path, args.fraction)    
    if status < 0:
        return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_folder', type=str, default='/scratch/gz5hp/encode3/cell/datasets')
    parser.add_argument('--file_list', type=str, default='/home/gz5hp/encode3_proj/all_file_list.txt')
    
    parser.add_argument('--token_folder', type=str, default='/scratch/gz5hp/encode3/cell/tokens')
    #parameters for hard tokenization
    parser.add_argument('--universe', type=str, default='/home/gz5hp/encode3_proj/GRCh38-universe.bed')
    parser.add_argument('--bedtools_path', type=str, default='/scratch/gz5hp/genomes/bedtools')
    parser.add_argument('--fraction', type=float, default=1.e-9)
