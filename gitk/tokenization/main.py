...
def hard_tokenization_main(src_folder, dst_folder, universe_file, fraction=1e-9, file_list=None, num_workers=10):
    timer = utils.Timer()
    start_time = timer.t()
    ...
    if nworkers <= 1:
        ...
        for param in args_arr:
            ...
            for f in allfiles:
                shutil.move(os.path.join(param.token_folder,f), os.path.join(dst_folder,f))
