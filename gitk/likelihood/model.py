...
def model_binomial(folderin, in_file, chrom, folderout, fout, file_no=None, start=0):
...
def make_models(
    chrom, folder_out, folder_in, in_file_start, in_file_end, in_file_core, file_no=None
):
    ...
    model_binomial(
        folder_in, in_file_start, chrom, folder_out, "start", file_no=file_no
    )
...
def make_models_multinomial(
    ...
    model[cov_nonzero, :] = (
        model[cov_nonzero, :]
        / np.sum(model[cov_nonzero, :], axis=1, keepdims=True)
        * model[cov_nonzero, 1].reshape((-1, 1))
    )
...
def main(
    ...
    bw_start = pyBigWig.open(os.path.join(coverage_folder, coverage_starts + ".bw"))
