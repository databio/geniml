...
class NBHMM(BaseHMM):
    ...
    def __init__(self, n_components=1, startprob_prior=1.0,
                 transmat_prior=1.0, failures_prior=0.0,
                 prob_prior=0.0,
                 failures_weight=0.0,
                 prob_weight=0.0,
                 algorithm="viterbi", random_state=None,
                 n_iter=10, tol=1e-2, verbose=False,
                 params="strp", init_params="strp",
                 implementation="log"):
    ...
    def _check(self):
        ...
        if self.prob_.shape != (self.n_components, n_features):
            raise ValueError(
                "proba_ must have shape (n_components, n_features)")
        if self.failures_.shape != (self.n_components, n_features):
            raise ValueError(
                "failures_ must have shape (n_components, n_features)")
    ...
    def _generate_sample_from_state(self, state, random_state):
        return random_state.negative_binomial(self.failures_[state], 
                                              self.prob_[state])

    def _compute_log_likelihood(self, X):
        return np.array([np.sum(nbinom.logpmf(X, failures, proba), axis=1)
                         for failures, proba in zip(self.failures_, self.prob_)]).T

    def _compute_likelihood(self, X):
        return np.array([np.prod(nbinom.pmf(X, failures, proba), axis=1)
                         for failures, proba in zip(self.failures_, self.prob_)]).T
...
class BetaHMM(BaseHMM):
    ...
    def __init__(self, n_components=1, startprob_prior=1.0,
                 transmat_prior=1.0, alfa_prior=0.0,
                 beta_prior=0.0,
                 alfa_weight=0.0,
                 beta_weight=0.0,
                 algorithm="viterbi", random_state=None,
    ...
    def _check(self):
        ...
        if self.alfa_.shape != (self.n_components, n_features):
            raise ValueError(
                "alfa_ must have shape (n_components, n_features)")
        if self.beta_.shape != (self.n_components, n_features):
            raise ValueError(
                "beta_ must have shape (n_components, n_features)")
    ...
    def _generate_sample_from_state(self, state, random_state):
        return random_state.beta(self.alfa_[state], 
                                              self.beta_[state])

    def _compute_log_likelihood(self, X):
        return np.array([np.sum(beta.logpdf(X, a, b), axis=1)
                         for a, b in zip(self.alfa_, self.beta_)]).T

    def _compute_likelihood(self, X):
        return np.array([np.prod(beta.pdf(X, a, b), axis=1)
                         for a, b in zip(self.alfa_, self.beta_)]).T
