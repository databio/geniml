...
class Model(ABC):
    ...
    def save_tras(self, out_folder):
        np.savetxt(os.path.join(out_folder, "trans_matrix.csv"),
                   self.trans_matrix)


class PoissonModel(Model):
    """ HMM model with Poisson emissions """
    def __init__(self, trans_matrix, lambdas_matrix,
                 init_para="", para="", save_matrix=True,
                 out_folder=""):
        """
        ...
        """
        Model.__init__(self, trans_matrix=trans_matrix,
                       init_para=init_para, para=para,
                       save_matrix=save_matrix)
        self.lambdas_matrix = lambdas_matrix
        if save_matrix:
            super().save_tras(out_folder)
            np.savetxt(os.path.join(out_folder, "lambdas_matrix.csv"),
                       self.lambdas_matrix)

    def make(self):
        """ Initialize HMM model """
        model = hmm.PoissonHMM(n_components=self.state_no, verbose=True,
...
class GaussianModel(Model):
    ...
    def __init__(self, trans_matrix, means_matrix, covars_matrix,
                 init_para="", para="", save_matrix=True,
                 out_folder=""):
        """
        ...
        if save_matrix:
            super().save_tras(out_folder)
            np.savetxt(os.path.join(out_folder, "covars_matrix.csv"),
                       self.covars_matrix)
            np.savetxt(os.path.join(out_folder, "means_matrix.csv"),
                       self.means_matrix)

    def make(self):
        """ Initialize HMM model """
        model = hmm.GaussianHMM(n_components=self.state_no, verbose=True,
                               init_params=self.init_para,
                               params=self.para)
...
class NBModel(Model):
    ...
    def __init__(self, trans_matrix, failures_matrix, prob_matrix,
                 init_para="", para="", save_matrix=True,
                 out_folder=""):
        """
        ...
        if save_matrix:
            super().save_tras(out_folder)
            np.savetxt(os.path.join(failures_matrix, "failures_matrix.csv"),
                       self.covars_matrix)
            np.savetxt(os.path.join(out_folder, "prob_matrix.csv"),
                       self.prob_matrix)

    def make(self):
        """ Initialize HMM model """
        model = NBHMM(n_components=self.state_no, verbose=True,
                             init_params=self.init_para,
                             params=self.para)
...
class BetaModel(Model):
    ...
    def __init__(self, trans_matrix, alfa_matrix, beta_matrix,
                 init_para="", para="", save_matrix=True,
                 out_folder=""):
        """
        ...
        if save_matrix:
            super().save_tras(out_folder)
            np.savetxt(os.path.join(out_folder, "alfa_matrix.csv"),
                       self.alfa_matrix)
            np.savetxt(os.path.join(out_folder, "beta_matrix.csv"),
                       self.beta_matrix)

    def make(self):
        """ Initialize HMM model """
        model = BetaHMM(n_components=self.state_no, verbose=True,
                             init_params=self.init_para,
                             params=self.para)
