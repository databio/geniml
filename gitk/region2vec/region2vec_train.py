...
def main(args):
    ...
    data_folder = os.path.join(save_dir, 'shuffled_datasets') # shuffled datasets are stored in the shuffled_datasets folder
    ...
    lr_scheduler = utils.lr_scheduler(args.init_lr, args.min_lr, args.num_shuffle,lr_info=lr_info,mode=args.lr_mode)
    ...
    for sidx in range(args.num_shuffle):
        ...
        est_time = (run_timer.t() - build_vocab_time) / (sidx+1) * args.num_shuffle + build_vocab_time
    ...
    utils.log('[{}] Training finished, training Time {}'.format(datetime.datetime.now().strftime("%x-%X"), utils.time_str(elasped_time)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_alg', help='training algorithm, select from [cbow, skip-gram]')
    parser.add_argument('--min_count', type=int, help='threshold of pruning the internal vocabulary')
    parser.add_argument('--neg_samples', type=int, help='number of noise words in negative sampling, usually between 5-20')
    parser.add_argument('--hier_softmax', default=False, action='store_true', help='if given, hierarchical softmax will be used')
    ...
    parser.add_argument('--lr_mode', type=str, choices=['milestone','linear'], help='milestone or linear')
    parser.add_argument('--update_vocab', type=str, default='once', help='[every] update at every epoch; [once] Update once since the vocabulary does not change')
