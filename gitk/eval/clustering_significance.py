...
def random_annotate_points(labels, num_per_cluster=10, min_num_per_cluster=1, max_font_size=20, min_font_size=8):
    ...
    for i,c in enumerate(cluster_labels):
        num = len(clusters[i])
        annotate_num = max(min(int(ratios[i]/ratios.max() * num_per_cluster),num),min(num,min_num_per_cluster))
...
def clustering_significance_test(model_path, embed_type, save_folder, Rscript_path, assembly, K_arr=[5,20,40], num_samples=1000, threshold=0.0001, num_workers=10, seed=0):
    for K in K_arr:
        target_folder = os.path.join(save_folder, 'Kmeans_{}'.format(K))
        clustering(model_path, embed_type, K, target_folder, seed=0)
    curr_folder = os.path.dirname(os.path.abspath(__file__))
    subprocess.call([Rscript_path, "{}/permutation_test.R".format(curr_folder), '--assembly', assembly, '--num_workers', str(num_workers), '--path', save_folder, '--num_samples', str(num_samples)])
    scores = []
    for K in K_arr:
        ...
        scores.append(score)
    print(model_path)
    print('(K, CCSI): '+' '.join(['({},{:.6f})'.format(K_arr[i], scores[i]) for i in range(len(K_arr))]))
    print('\n')
    return scores


def clustering_significance_test_batch(batch, save_folder, Rscript_path, assembly, K_arr=[5,20,40,60,100], num_samples=1000, threshold=0.0001, num_workers=10, seed=0):
    scores_batch = []
    for i, (model_path, embed_type) in enumerate(batch):
        target_folder = os.path.join(save_folder, 'model_{}'.format(i))
        scores = clustering_significance_test(model_path, embed_type, target_folder, Rscript_path, assembly, K_arr, num_samples, threshold, num_workers, seed)
...
def cst_eval(batch, save_folder, Rscript_path, assembly, K_arr=[5,20,40,60,100], num_samples=1000, threshold=0.0001, num_runs=20, num_workers=10):
    avg_ranks_arr = []
    for seed in range(num_runs):
        target_folder = os.path.join(save_folder, 'cst_seed{}'.format(seed))
        scores_batch, avg_ranks = clustering_significance_test_batch(batch, target_folder, Rscript_path, assembly, K_arr, num_samples, threshold, num_workers, seed)
...
def cst_plot(avg_ranks_arr, row_labels=None, legend_pos=(0.25, 0.6), filename=None):
    ...
    ax.errorbar(range(1,len(mean_rank)+1), mean_rank, yerr=std_rank,fmt='o',ms=10, mfc=cmaplist[1], mec=cmaplist[8], ecolor=cmaplist[2], elinewidth=3, capsize=5)
    ...
    legend = ax.legend(labels=['CCSI average rank','CCSI rank standard deviation'], handles=patches, bbox_to_anchor=legend_pos, loc='center left', borderaxespad=0, fontsize=12, frameon=True)
    ax.grid('on')
